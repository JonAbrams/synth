var fs = require('fs'),
    ...
    BowerStandardRenderer = require('../node_modules/bower/lib/renderers/StandardRenderer.js');
...
function loadApp () {
  try {
    ...
    if (err.code === 'MODULE_NOT_FOUND') {
      console.error('No synth app detected (missing synth.json).');
      process.exit(1);
    }
  }
}
...
exports.install = function (command, args) {
  ...
  if (bowerComponentIndex || both) {
    ...
    var bowerInstall = bower.commands[command](bowerComponents, { save: true });
               
    bowerInstall.on('log', function(log) {
        /**
        * This is how bower renders the status to the CLI upon packages being installed
        */
        var bowerStdRenderer = new BowerStandardRenderer({}, {});
        bowerStdRenderer.log(log);
    });
