...
exports.index = function (req, res) {
  ...
  // Load list of assets
  Promise.resolve(manifests['jsFiles'] ||
    Promise.all(
      ['jsFiles', 'cssFiles'].map(function (filename) {
        return fs.readFileAsync(
          path.join(process.cwd(), 'front/dist', filename)
        ).then(function (content) {
          return manifests[filename] = content.toString('utf8')
            .replace(/\n$/, '').split('\n');
        });
      })
    )
